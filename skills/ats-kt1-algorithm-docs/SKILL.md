...
- 输入输出表的中列只写文件级信息，格式为“简短中文文件介绍＋简短文件名”；文件名不得带 `input/`、`output/`、`/app/data/`、宿主机目录等路径前缀。中文文件介绍直接使用名称，不添加“（节选）”“（结构节选）”等括号补充。右列只写文件内部字段或变量，格式为“中文字段介绍＋变量名＋类型”，例如“最小标准差下限↵min_sigma (float)”；不得写“内容：…”、示例值、实际值或文件路径，也不得把文件介绍和字段列表混写在同一单元格。
