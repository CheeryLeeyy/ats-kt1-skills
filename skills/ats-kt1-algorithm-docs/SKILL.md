...
先生成并校验测试说明，再填写模型原理说明。以同一算法的旧模型原理说明保持算法设计与模块细节一致，从已校验测试 JSON 直接复用输入、输出、格式、字段和指标说明，并使用步骤 0 确定的统一名称。只复用算法实际读取或生成的数据文件；算法根目录中仅用于启动 Docker 的 `params.json`、GPU、共享内存、镜像入口等运行配置不得进入模型原理说明的输入输出表，算法在 `input/` 内实际读取的同名业务数据仍按真实内容填写。生成器写入新模型原理说明前必须将已有同名文档归档为 `-old`、`-old-2`、`-old-3`。最后使用 LibreOffice 打开或转换两类 Word，逐页检查版式并清理临时件和本次加载的 Docker 对象。
...
- 输入输出表的中列只写文件级信息，格式为“简短中文文件介绍＋真实文件名”；中文文件介绍直接使用名称，不添加“（节选）”“（结构节选）”等括号补充。右列只写文件内部字段或变量，格式为“中文字段介绍＋变量名＋类型＋具体内容”。不得把文件介绍和字段列表混写在同一单元格。
...
旧模型原理说明不存在或没有可识别图片时，跳过提取命令，`--missing-only` 会生成两幅图。若只识别出其中一类，保留提取结果并只生成另一类；不得在提取后因图片内容审查而删除旧图。人工检查仅用于确认框架图与流程图分类是否正确、图片是否完整写入新文档，不作为替换旧图的理由。
